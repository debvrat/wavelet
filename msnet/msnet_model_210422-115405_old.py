...
class msNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=1):
        ...
        self.wav3 = waveT(3) ## added by @dv
        self.wav4 = waveT(4) ## added by @dv
    ...
    def forward(self, x,w):
        ...
        if w is not None:
            ...
            fcat_2=torch.cat((so22, w1[0].cuda(),w1[1].cuda(), w1[2].cuda()), dim=1)
            so2 =self.fusew_2(fcat_2)
            w2 = self.wav1(so2)
            
            so33 = self.dsn3(conv3)#1/4
            fcat_3=torch.cat((so33, w2[0].cuda(),w2[1].cuda(), w2[2].cuda()), dim=1)
            so3 =self.fusew_3(fcat_3)
            w3 = self.wav1(so3)
            
            so44 = self.dsn4(conv4)#1/8
            fcat_4=torch.cat((so44, w3[0].cuda(),w3[1].cuda(), w3[2].cuda()), dim=1)
           ...
           # fcat_5=torch.cat((so55, w['cA4']), dim=1)
            ...
            fcat_5=torch.cat((so5, w4[0].cuda(),w4[1].cuda(), w4[2].cuda()), dim=1) # @dv: added
