...
class msNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=1):
        ...
        self.conv2 = DoubleConv(128, 128) #1/2
        self.conv3 = DoubleConv(256, 256) #1/4
        self.conv4 = DoubleConv(512, 512) #1/8
        self.conv5 = DoubleConv(1024, 512) #1/16

        self.wav1 = waveT(1) ## added by @dv
    ...
    def forward(self, x,w):
        ...
        wav1 = self.wav1(conv1)
        conv2 = self.conv2(torch.cat((wav1['cH1'],wav1['cV1']),dim=1))#1/2 #128
        wav2 = self.wav1(conv2)
        conv3 = self.conv3(torch.cat((wav2['cH1'],wav2['cV1']),dim=1))#1/4 #256
        wav3 = self.wav1(conv3)
        conv4 = self.conv4(torch.cat((wav3['cH1'],wav3['cV1']),dim=1))#1/8 #512
        wav4 = self.wav1(conv4)
        conv5 = self.conv5(torch.cat((wav4['cH1'],wav4['cV1']),dim=1))#1/16 #512
