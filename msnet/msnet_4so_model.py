### mainly, fuse 4 side outputs, rather than 5. Ignore So-WT1 written below
### So-WT1 network
...
class msNet_4so(nn.Module):
    def __init__(self, n_channels=3, n_classes=1):
        super(msNet_4so, self).__init__()
        ...
        # self.fuse = nn.Conv2d(5, 1, 1) # commented by @dv to concat 4 SOs
        self.fuse = nn.Conv2d(4, 1, 1) # to use just 4 side outputs (SOs) @dv
        
        self.wav1 = waveT(1,'db1') ## added by @dv
    ...
    def forward(self, x,w):
        ...
        # fusecat = torch.cat((so1, so2, so3, so4, so5), dim=1) # commented by @dv
        fusecat = torch.cat((so1, so2, so3, so4), dim=1) # by @dv
        fuse = self.fuse(fusecat)
        # results = [so1, so2, so3, so4, so5, fuse] #commented by @dv
        results = [so1, so2, so3, so4, fuse] # by @dv
