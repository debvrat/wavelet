...
class msNet(nn.Module):
    ...
    def forward(self, x,w):
        ...
        if w is not None:
            ...
            so1 = self.dsn1(conv1)
            so22 = self.dsn2(F.pad(conv2,(1,0,0,1),'constant',0))#1/2
            fcat_2=torch.cat((so22, w['cH1'],w['cV1'], w['cD1']), dim=1)
            so2 =self.fusew_2(fcat_2)

            so33 = self.dsn3(F.pad(conv3,(1,0,0,1),'constant',0))#1/4
            fcat_3=torch.cat((so33, w['cH2'],w['cV2'], w['cD2']), dim=1)
            so3 =self.fusew_3(fcat_3)

            so44 = self.dsn4(F.pad(conv4,(1,0,0,1),'constant',0))#1/8
            fcat_4=torch.cat((so44, w['cH3'],w['cV3'], w['cD3']), dim=1)
            so4 =self.fusew_4(fcat_4)

            so5 = self.dsn5(F.pad(conv5,(1,0,0,1),'constant',0))#1/16
           # fcat_5=torch.cat((so55, w['cA4']), dim=1)
            #so5 =self.fusew_5(fcat_5)
            
            fcat_5=torch.cat((so5, w['cH4'],w['cV4'], w['cD4']), dim=1) # @dv: added
